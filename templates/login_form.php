    ...
    <form class="form container <?php if (count($form_errors) > 0) :
      ...
      <h2>Вход</h2>
        ...
        <input id="email" type="text" name="email" placeholder="Введите e-mail"
               value="<?=xssProtection($incoming_data['email']); ?>">
        <span class="form__error"><?=$form_errors['email'];?></span>
      </div>
      <div class="form__item form__item--last <?php if (isset($form_errors['password'])) :?>
                                                    form__item--invalid
                                              <?php endif; ?>">
        <label for="password">Пароль <sup>*</sup></label>
        <input id="password" type="password" name="password" placeholder="Введите пароль"
               value="<?=xssProtection($incoming_data['password']); ?>">
